...
frequency_handle <- tweet %>%
  ...
  #count(month_year, handle)

ggplot(frequency_handle, aes(as.Date(date), n), group = handle) + 
   geom_line(size = 1, alpha = 0.7, aes(color = handle)) +
...
```{r}
name <- c("clinton", "hillary", "Clinton", "Hillary")
...
#personal concerns
model4 <- glm(is_trump~relig+death+achieve+female, family=binomial(link='logit'),data=nrc_tweet)
