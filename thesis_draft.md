...
```
    ...
    ## # ... with 3,235 more rows, and 6 more variables: text <chr>,
...
``` r
#how sentiment changes over time (mean)
average %>%
   ggplot(aes(as.Date(date), Mean), group = handle) + 
   geom_line(size = 0.5, alpha = 0.7, aes(color = handle)) +
   scale_colour_manual(values = c("springgreen4", "firebrick3")) +
   ...
   theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
   labs(title = "Figure 8: Change of sentiment over time") +
   ggsave(file="sentiment_change_mean.png") 
```
...
nrc_clinton <- nrc_tweet %>% 
  filter(handle == "clinton") %>%
  group_by(date) %>%
  summarise(negative_mean = mean(negative)*-1,
            positive_mean = mean(positive)) %>% melt %>%
  mutate(handle = "clinton")
...
nrc <- rbind(nrc_clinton, nrc_trump)

names(nrc) <- c("timestamp", "sentiment", "meanvalue", "handle")

nrc$sentiment = factor(nrc$sentiment,levels(nrc$sentiment)[c(2,1)])

ggplot(data = nrc, aes(x = as.Date(timestamp), y = meanvalue, group = sentiment)) +
      ...
      ggtitle("Figure 9: Change of sentiment over time") +
      ggsave(file="sentiment_separate_overtime.png")
```
...
![](thesis_draft_files/figure-markdown_github/unnamed-chunk-15-1.png)
